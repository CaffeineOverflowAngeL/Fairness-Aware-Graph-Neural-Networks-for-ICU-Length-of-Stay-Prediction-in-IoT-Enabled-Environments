...
class Generator():
    ...
    def generate_adm(self):
        ...
        data[['days', 'dummy','hours']] = data['los'].str.split(' ', expand=True) # old version: split(' ', -1, expand=True)
        data[['hours','min','sec']] = data['hours'].str.split(':', expand=True) # old split(' ', -1, expand=True)
    ...
    def generate_proc(self):
        ...
        proc[['start_days', 'dummy','start_hours']] = proc['event_time_from_admit'].str.split(' ', expand=True) # old: str.split(' ', -1, expand=True)
        proc[['start_hours','min','sec']] = proc['start_hours'].str.split(':', expand=True) # old: str.split(':', -1, expand=True)
    ...
    def generate_chart(self):
        ...
        for chart in tqdm(pd.read_csv("./data/features/preproc_chart_icu.csv.gz", compression='gzip', header=0, index_col=None,chunksize=chunksize)):
            ...
            chart[['start_days', 'dummy','start_hours']] = chart['event_time_from_admit'].str.split(' ', expand=True) #old: str.split(' ', -1, expand=True)
            chart[['start_hours','min','sec']] = chart['start_hours'].str.split(':', expand=True) #old: str.split(':', -1, expand=True)
            ...
            if final.empty:
                ...
                final = pd.concat([final, chart], ignore_index=True) # old: final=final.append(chart, ignore_index=True)
    ...
    def generate_meds(self):
        ...
        meds[['start_days', 'dummy','start_hours']] = meds['start_hours_from_admit'].str.split(' ', expand=True) #old: str.split(' ', -1, expand=True)
        meds[['start_hours','min','sec']] = meds['start_hours'].str.split(':', expand=True) #old: str.split(' ', -1, expand=True)
        meds['start_time']=pd.to_numeric(meds['start_days'])*24+pd.to_numeric(meds['start_hours'])
        meds[['start_days', 'dummy','start_hours']] = meds['stop_hours_from_admit'].str.split(' ', expand=True) # old: str.split(' ', -1, expand=True)
        meds[['start_hours','min','sec']] = meds['start_hours'].str.split(':', expand=True) # old: str.split(':', -1, expand=True)
            ...
            ###MEDS
             if(self.feat_med):
                ...
                if final_meds.empty:
                    ...
                    final_meds = pd.concat([final_meds, sub_meds], ignore_index=True) # old: final_meds=final_meds.append(sub_meds)
            ...
            ###PROC
             if(self.feat_proc):
                ...
                if final_proc.empty:
                    ...
                    final_proc = pd.concat([final_proc, sub_proc], ignore_index=True) # old: final_proc=final_proc.append(sub_proc)
             ...
             if(self.feat_out):
                ...
                if final_out.empty:
                    ...
                    final_out = pd.concat([final_out, sub_out], ignore_index=True) # old: final_out=final_out.append(sub_out)
             ...
             if(self.feat_chart):
                ...
                if final_chart.empty:
                    ...
                    final_chart = pd.concat([final_chart, sub_chart], ignore_index=True) # old: final_chart=final_chart.append(sub_chart)
