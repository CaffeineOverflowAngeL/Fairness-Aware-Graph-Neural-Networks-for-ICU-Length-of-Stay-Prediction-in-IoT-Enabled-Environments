...
from benchmark_utils import mimic_models_constructor, vocab_loader, logger_utils, mimic_loader, comp_utils, fairness
...
parser.add_argument("--fairness", action="store_true", default=False)
parser.add_argument("--batch-size", type=int, default=128)
parser.add_argument("--total-epochs", type=int, default=5)
parser.add_argument("--lr-decay-milestones", default="8", type=str, help="milestones for learning rate decay")
...
# Record Keeping
parser.add_argument("--days", type=str,  default="3days", choices=["3days", "7days"])
...
def test(
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm(test_loader, desc="Evaluating", unit="batch")):
            ...
            # Fairness
            args.eth.extend(data["demo"][:,1].tolist())
            args.gender.extend(data["demo"][:,0].tolist())
            args.ins.extend(data["demo"][:,2].tolist())
            args.age.extend(data["demo"][:,3].tolist())
            #
            args.prob.extend(out.data.cpu().numpy())
            args.truth.extend(target.data.cpu().numpy())
            args.logits.extend(logits.data.cpu().numpy())
...
def inference_throughput_eval(model, example_inputs, device=None):
    ...
    model.to(device)
    print("I am in Test:", example_inputs.shape)
...
def save_output(output_filename):
    reversed_eth = {args.eth_vocab[key]: key for key in args.eth_vocab}
    reversed_gender = {args.gender_vocab[key]: key for key in args.gender_vocab}
    reversed_age = {args.age_vocab[key]: key for key in args.age_vocab}
    reversed_ins = {args.ins_vocab[key]: key for key in args.ins_vocab}
    
    args.eth=list(pd.Series(args.eth).map(reversed_eth))
    args.gender=list(pd.Series(args.gender).map(reversed_gender))
    args.age=list(pd.Series(args.age).map(reversed_age))
    args.ins=list(pd.Series(args.ins).map(reversed_ins))
    
    output_df=pd.DataFrame()
    output_df['Labels']=args.truth
    output_df['Prob']=args.prob
    output_df['Logits']=args.logits
    output_df['ethnicity']=args.eth
    output_df['gender']=args.gender
    output_df['age']=args.age
    output_df['insurance']=args.ins
...
def get_model(model_type, embed_size, rnn_size):
    if model_type=='Time-series-LSTM':
        net = mimic_models_constructor.LSTMBase(args.device,
                            ...
                            batch_size=args.batch_size, fairness=args.fairness) 
        ...
        net = mimic_models_constructor.CNNBase(args.device,
                            ...
                            batch_size=args.batch_size, fairness=args.fairness, kernel_size=args.kernel_size) 
        ...
        net = mimic_models_constructor.GNNBase(args.device,
                            ...
                            args.eth_vocab_size,args.gender_vocab_size,args.age_vocab_size,args.ins_vocab_size,
                            args.modalities,
                            embed_size=embed_size,latent_size=args.latent_size,
                            batch_size=args.batch_size, fairness=args.fairness) 
...
def main():
    ...
    # Logger
    if args.mode == "pretrain":
        args.output_dir = os.path.join(args.output_dir, args.days, args.lookback_window, args.timebucket_size)
    ...
    args.logger.info("PyTorch Version: {}".format(pytorch_version))
    ...
    lab_flag = False
    if args.mode == "pretrain": 
        args.cond_vocab_size,args.proc_vocab_size,args.med_vocab_size,args.out_vocab_size,args.chart_vocab_size,args.lab_vocab_size,args.eth_vocab,args.gender_vocab,args.age_vocab,args.ins_vocab=vocab_loader.init(diag_flag,proc_flag,out_flag,chart_flag,med_flag,lab_flag, args.dataset+'dict/')
    else:
        args.cond_vocab_size,args.proc_vocab_size,args.med_vocab_size,args.out_vocab_size,args.chart_vocab_size,args.lab_vocab_size,args.eth_vocab,args.gender_vocab,args.age_vocab,args.ins_vocab=vocab_loader.init_read(diag_flag,proc_flag,out_flag,chart_flag,med_flag,lab_flag, args.dataset+'dict/')
    ...
    ### Data Loader ###
    train_loader, test_loader = mimic_loader.mimic_dataloaders(args.dataset+'csv/labels.csv', args.dataset+'csv', 
    ...
    if args.mode == "pretrain":
        ...
        output_filename = args.output_dir+'/outputDict/'+str(args.model_type)+'-'+str(args.fairness)
        fairness_report_filename = "./fairness_evals/"+str(args.model_type)+'-'+str(args.fairness)
        ...
        acc, val_loss, eval_metrics = test(model, test_loader, device=args.device)
        save_output(output_filename)
        # Extract each metric from eval_metrics for easy formatting
        ...
        # Check if the CSV file exists, then append or create
        if Path(csv_file_path).is_file():
            # Append to existing CSV
            row_df.to_csv(csv_file_path, mode='a', header=False, index=False)
        else:
            # Create a new CSV with headers
            row_df.to_csv(csv_file_path, mode='w', header=True, index=False)

        fairness.fairness_evaluation(inputFile=output_filename,outputFile=fairness_report_filename)
